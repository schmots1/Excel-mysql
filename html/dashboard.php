...
// Delta section
echo "<tr><td valign='top'><table border=1>";
echo "<tr><td colspan=2 bgcolor=lightgrey>Changed Volumes</td></tr>";
$sql = "SHOW DATABASES LIKE 'delta-" . $project[1] . "'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
$sql = "select count(*) from `up-" . $project[1] . "`.`Volumes` where name not in (select name from `delta-" . $project[1] . "`.`Volumes`)";
$result = $conn->query($sql);
while($row=$result->fetch_assoc()) {
echo "<tr><td>Removed Volumes</td><td>" . $row['count(*)'] . "</td></tr>";
}
$sql = "select count(*) from `delta-" . $project[1] . "`.`Volumes` where name not in (select name from `up-" . $project[1] . "`.`Volumes`)";
$result = $conn->query($sql);
while($row=$result->fetch_assoc()) {
echo "<tr><td>Added Volumes</td><td>" . $row['count(*)'] . "</td></tr>";
}
}
else{
echo "<tr><td colspan=2>Create and convert a delta</td></tr>";
}
echo "</table>";
